...
echo "         SEQUENTIAL WITH d=0.01         " | tee -a "$RESULTS_FILE"
echo "========================================" | tee -a "$RESULTS_FILE"
echo "" | tee -a "$RESULTS_FILE"
...
for INPUTS in 8 16 32 64 128; do
    ...
    output=$(/usr/bin/time -l ../../build/Sort --inline "$vector" --delta 0.01 --permutation --tieoffset 2> >(grep "peak memory footprint"))
