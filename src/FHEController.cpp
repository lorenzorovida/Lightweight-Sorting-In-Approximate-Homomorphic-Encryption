...
static inline void print_moduli_chain(const DCRTPoly& poly){
...
Ptxt FHEController::encode_repeated(const vector<double> &vec, int level, int num_slots, int repetitions) {
    ...
    Ptxt p = context->MakeCKKSPackedPlaintext(repeated, 1, level, nullptr, num_slots);
    p->SetLength(num_slots);
...
/**
 * Perform a slot-wise addition operation on two ciphertext-plaintext
 *
 * @param a The ciphertext.
 * @param b The plaintext.
 * @return The result of addition operation on a and b.
 */
...
//Vec in expanded, mat rowwise
Ctxt FHEController::vec_mat(const Ctxt &vec, const Ctxt &mat, int n) {
    Ctxt mul = context->EvalMult(vec, mat);
 ...
 * @return
 */
Ctxt FHEController::min(const Ctxt &a, const Ctxt &b, int degree) {
...
Ctxt FHEController::sinc_scaled(const Ctxt &in, int poly_degree, int n) {
    return context->EvalChebyshevFunction([n](double x) -> double { return sin(3.14159265358979323846 * x * n) / (3.14159265358979323846 * x * n) * (1/n); },
                                          in,
                                          -1,
                                          1, poly_degree); //-1 because of an OpenFHE bug!!
}
...
void FHEController::print_expanded(const Ctxt &c, int slots, string prefix) {
    if (slots == 0) {
        slots = c->GetSlots();
    }
    ...
    for (int i = 0; i < slots * slots; i += slots) {
        string segno = "";
        if (v[i] > 0) {
            segno = "";
        } else {
